...
- **System Entity:** A special Entity with `type='system'` exists to house global roles like **Super Admin**. This ensures that even global roles are mapped within the database's Primary Key constraints for the `model_has_roles` table.
...
2.  **Schema:**
    -   `entities`: Parent grouping (User owned). Acts as RBAC Team. One entity has `type='system'` for Super Admin roles.
...
3.  **Conventions:**
    ...
    -   Super Admin roles are scoped to the 'system' type Entity.
```